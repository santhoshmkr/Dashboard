...
const UpdateProduct = ({ categories = [] }) => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await axios.get(`http://localhost:6060/update-Product/${id}`);
        setProductData(response.data);
        setFields(response.data.variants);
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await axios.put(`http://localhost:6060/update-Product/${id}`, data, {
        headers: {
          'Content-Type': 'application/json',
        },
      });
      console.log('productData Updated:', res.data);
      Navigate('/Dashboard')
    } catch (err) {
              ...
              <div>
                ...
                <div className="flex">
                  <input
                    ...
                    name="weight"
                    value={productData.shippingDetails.weight}
                    onChange={(e) =>
                      setProductData({
                        ...productData,
                        shippingDetails: {
